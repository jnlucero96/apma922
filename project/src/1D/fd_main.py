...
def main():
    ...
    print(f"Using Central Space {scheme_to_name[scheme]} method")
    ...
    if not steady:
        fd_mod.fdiff.get_fd_fwd(
            nsteps, dt, scheme, check_step, problem.D, problem.dx, problem.dy,
            drift1, drift2, p_initial, p_trace, nchecks, problem.n, problem.m
            )
    else:
        fd_mod.fdiff.get_fd_steady(
            dt, scheme, check_step, problem.D, problem.dx, problem.dy,
            drift1, drift2, p_initial, p_ss, refarray, problem.n, problem.m
            )
